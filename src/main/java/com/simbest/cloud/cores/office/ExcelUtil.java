 ...
 */
package com.simbest.cloud.cores.office;
...
public class ExcelUtil<T> {
    ...
    private void setFieldValue(T entity, String c, Class<?> fieldType, Field field) throws IllegalAccessException {
        if (StringUtils.isNotEmpty(c) && (String.class == fieldType)) {
            ...
            field.set(entity, com.simbest.cloud.cores.utils.DateUtil.parseDate(c));
        ...
        } else if (StringUtils.isNotEmpty(c) &&  (BigDecimal.class == fieldType)) {
            if(c != null && c.length() > 0){
                field.set(entity, new BigDecimal(c));
            }
        } else if (StringUtils.isNotEmpty(c) && (LocalDateTime.class == fieldType)) {
            field.set(entity, LocalDateTime.parse(c, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
            ...
            if (rows > 0) {// 有数据时才处理
                ...
                for (int i = 1; i < rows; i++) {// 从第2行开始取数据,默认第一行是表头.
                    ...
                    for (int j = 0; j <= cellNum; j++) {
                        ...
                        entity = (entity == null ? clazz.getConstructor().newInstance() : entity);// 如果不存在实例则新建.
    ...
    public boolean exportExcel(List<T> lists[], String sheetNames[], OutputStream output, String isTagValue) {
        ...
        for (int ii = 0; ii < lists.length; ii++) {
            ...
            for (int i = 0; i < fields.size(); i++) {
                ...
                //如果穿了列宽，则按照传的参数来处理
                int iLength =  (attr.width() > 0 ? attr.width() : attr.name().getBytes().length * 2 ) * 256;
                intArray[i] = iLength;//初始化列宽
                sheet.setColumnWidth(col, iLength);//手动设置列宽
                // 如果设置了提示信息则鼠标放上去提示.
