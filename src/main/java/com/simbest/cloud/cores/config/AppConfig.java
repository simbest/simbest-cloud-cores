...
public class AppConfig {
    ...
    // 是否开启验证码功能
    @Value("${app.captcha.enable:false}")
...
//    private String datasourcePassword;
    ...
    public void init() {
        ...
        for (PropertySource<?> source : sources) {
            if (source instanceof EnumerablePropertySource) {
                ...
                for (String name : propertySource.getPropertyNames()) {
                    if(name.startsWith("spring.shardingsphere.datasource")){
                        if(name.contains("url") || name.contains("username") || name.contains("password")){
                            datasourcePropertyMap.put(name, propertySource.getProperty(name));
                        }
                    }
                }
            }
        ...
        for (Map.Entry<String, Object> m : datasourcePropertyMap.entrySet()) {
            log.info("数据库配置项【{}】：配置值【{}】", m.getKey(), m.getValue());
        }
