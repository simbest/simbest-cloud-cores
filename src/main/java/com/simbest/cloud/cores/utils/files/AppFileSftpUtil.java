...
public class AppFileSftpUtil {
    ...
    private String ftpUpload(String directory, String fileName, InputStream inputStream) {
        try {
            ...
            String fullFilePath = directory+ApplicationConstants.SLASH+fileName;
            return fullFilePath;
        } catch (Exception e){
    ...
    private String sftpUpload(String directory, String fileName, InputStream input) {
        try{
            try {
                ...
                if(StringUtils.startsWith(directory, ApplicationConstants.SLASH)){
                    directory = StringUtils.removeFirst(directory, ApplicationConstants.SLASH);
                }
                String[] folders = directory.split( ApplicationConstants.SLASH );
                try{
                    //第一个提供的目录一定要有权限进入并且读写
                    sftpChannel.cd(ApplicationConstants.SLASH+folders[0]);
                }
                catch (Exception e1){
                    throw new IllegalArgumentException(String.format("SFTP无法进入%s目录，路径%s提供错误，请检查路径或读写访问权限！", ApplicationConstants.SLASH+folders[0], directory));
                }
    ...
    private byte[] ftpDownload2Byte(String directory, String fileName) {
        try {
            ...
            ftp.close();
            if(null == fileData || fileData.length == ApplicationConstants.ZERO){
    ...
    private void sftpDeleteFile(String directory, String fileName){
        try {
            ...
            sftpChannel.rm(directory+ApplicationConstants.SLASH+fileName);
            sftpDisconnect();
