...
/**
 * 先来生成私钥：openssl genrsa -out rsa_private_key_2048.pem 2048
 * 接下来根据私钥生成公钥：openssl rsa -in rsa_private_key_2048.pem -out rsa_public_key_2048.pem -pubout
 * 私钥还不能直接被使用，需要进行PKCS#8编码：openssl pkcs8 -topk8 -in rsa_private_key_2048.pem -out pkcs8_rsa_private_key_2048.pem -nocrypt
 * 至此，可用的密钥对已经生成好了，私钥使用pkcs8_rsa_private_key_2048.pem，公钥采用rsa_public_key_2048.pem。
 *
 * 参考资料：https://blog.csdn.net/chaijunkun/article/details/7275632
 */
@Slf4j
@Component
public class Rsa2048Util {
    ...
    public static String encryptSource(String source) {
        //  http://commons.apache.org/proper/commons-codec/ : org.apache.commons.codec.binary.Base64
        // sun.misc.BASE64Encoder
        try {
            byte[] binaryData = rsaUtil.encrypt(getPublicKey(), source.getBytes());
    ...
    public static String decryptCode(String code) {
        ...
        try {
            return rsaUtil.doDecryptCode(code);
        } catch (Exception e) {
            int codeArrayLength = code.split(" ").length;
            for(int i=0; i<codeArrayLength; i++){
