...
public class AppFileSftpUtil {
    ...
    public void deleteFile(String fileFullPath){
        ...
        String directory = StringUtils.substringBeforeLast(fileFullPath, SLASH);
        String fileName = StringUtils.substringAfterLast(fileFullPath, SLASH);
        switch (serverUploadLocation) {
            case ftp:
    ...
    private void ftpDeleteFile(String directory, String fileName){
        try {
            Ftp ftp = new Ftp(host, port, username, password, StandardCharsets.UTF_8 );
            boolean ret = ftp.delFile(directory + SLASH + fileName);
            if(!ret){
