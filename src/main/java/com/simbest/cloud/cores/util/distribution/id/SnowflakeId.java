 ...
 */
package com.simbest.cloud.cores.util.distribution.id;
 ...
 * 参考： https://blog.csdn.net/qq_26545503/article/details/105717826
 */
@Slf4j
...
//        }
    ...
    protected static long getDataCenterId(long tempMaxDataCenterId) {
        ...
        try {
            ...
            //log.warn("主键生成策略工具中的IP地址为：【{}】",ip);
            NetworkInterface network = NetworkInterface.getByInetAddress(ip);
            Enumeration<NetworkInterface> interfaceEnumeration = NetworkInterface.getNetworkInterfaces();
            Iterator<NetworkInterface> unmodifiableIterator = Iterators.forEnumeration(interfaceEnumeration);
            List<NetworkInterface> networkInterfaces = Lists.newArrayList(unmodifiableIterator);
            if (CollectionUtil.isNotEmpty(networkInterfaces)){
                List<NetworkInterface> networkInterfaceList = networkInterfaces.stream().filter(networkInterface -> {
                    try {
                        return ArrayUtil.isNotEmpty(networkInterface.getHardwareAddress());
                    } catch (SocketException e) {
                        e.printStackTrace();
                    }
                    return false;
                }).collect(Collectors.toList());
                //log.warn("1-主键生成策略工具中的MAC地址为：【{}】", JacksonUtils.obj2json(networkInterfaceList));
                byte[] mac = CollectionUtil.getFirst(networkInterfaceList).getHardwareAddress();
                //log.warn("2-主键生成策略工具中的MAC地址为：【{}】", JacksonUtils.obj2json(mac));
           ...
           /* if (network == null) {
                ...
                log.warn("主键生成策略工具中的MAC地址为：【{}】", JacksonUtils.obj2json(mac));
                id = ((0x000000FF & (long) mac[mac.length - 1])
                        | (0x0000FF00 & (((long) mac[mac.length - 2]) << 8))) >> 6;
                id = id % (tempMaxDataCenterId + 1);
            }*/
    ...
    public static void testProductIdByMoreThread(int dataCenterId, int workerId, int n) throws InterruptedException {
        ...
        for(int i=0;i<10;i++){
            //用线程名称做map key.
            idFactories.put("snowflake"+i,new SnowflakeId(workerId, threadNo++));
        }
        for(int i=0;i<10;i++){
    ...
    public Serializable generate(SharedSessionContractImplementor sharedSessionContractImplementor, Object o) throws HibernateException {
        String prefix = ObjectUtil.getEntityIdPrefixVaue(o);
        if(StringUtils.isNotEmpty(prefix)){
            return prefix + getInstance().nextId();
        }
        else {
            return getInstance().nextId() + ""; //避免科学计数法显示数字
        }
    }
