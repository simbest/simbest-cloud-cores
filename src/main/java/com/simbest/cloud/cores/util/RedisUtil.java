...
/** -------------------key相关操作--------------------- */
    ...
    public static Long delete(Collection<String> keys) {
        Collection<String> realKeys = new HashSet<>();
        for (String k : keys) {
    ...
    public static <T> void setBean(String key, T obj) {
        cacheUtils.redisTemplate.opsForValue().set(prefix + key, JSONUtil.toJsonStr(obj));
    }

    public static <T> void setBeanGlobal(String key, T obj) {
        cacheUtils.redisTemplate.opsForValue().set(key, JSONUtil.toJsonStr(obj));
    }
    ...
    public static <T> void setBean(String key, T obj, int seconds) {
        cacheUtils.redisTemplate.opsForValue().set(prefix + key, JSONUtil.toJsonStr(obj), seconds, TimeUnit.SECONDS);
    }

    public static <T> void setBeanGlobal(String key, T obj, int seconds) {
        cacheUtils.redisTemplate.opsForValue().set(key, JSONUtil.toJsonStr(obj), seconds, TimeUnit.SECONDS);
    }
...
//	public static <T> T getBeanGlobal(String key, TypeReference<T> listType) {
//		String value = cacheUtils.redisTemplate.opsForValue().get(key);
//		if (null == value) {
//			return null;
//		} else {
...
//	}
    ...
    public static void multiSet(Map<String, String> maps) {
        Map<String, String> realMaps = new HashMap<>();
    ...
    public static boolean validateAppSMSCode(String key, String smsCode) {
        ...
        if (StringUtils.isNotEmpty(cacheCode) && cacheCode.equals(smsCode)) {
            expire(SMS_VERIFICATION_CODE_PRIFIEX + key, 0, TimeUnit.SECONDS);
            return true;
        } else {
            return false;
        }
