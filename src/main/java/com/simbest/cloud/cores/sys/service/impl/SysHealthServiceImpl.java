...
public class SysHealthServiceImpl implements ISysHealthService, IHeartTestService {
    ...
    public SysHealth redisCheck(){
        log.info("开始对Redis节点【{}】进行可用性测试", redisConfiguration.getRedisClusterNodes());
        if (redisConfiguration.getRedisClusterNodes().split(ApplicationConstants.COMMA).length == 1) {
            String redisHost = redisConfiguration.getRedisClusterNodes().split(ApplicationConstants.COLON)[0];
            Integer redisPort = Integer.valueOf(redisConfiguration.getRedisClusterNodes().split(ApplicationConstants.COLON)[1]);
            return redisCheckAvailable(redisHost, redisPort);
        } else {
            SysHealth sysHealth = SysHealth.builder().result(true).build();
            StringBuffer sb = new StringBuffer();
            String[] redisNodes = redisConfiguration.getRedisClusterNodes().split(ApplicationConstants.COMMA);
