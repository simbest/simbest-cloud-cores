...
public interface SysDictValueRepository extends LogicRepository<SysDictValue, String> {
    ...
    @Query (value = "select dv from SysDictValue dv where dv.dictType=:dictType and dv.name=:name and dv.blocid is null and dv.corpid is null and dv.enabled=true")
    SysDictValue findByDictTypeAndName(@Param ("dictType") String dictType, @Param ("name") String name);
