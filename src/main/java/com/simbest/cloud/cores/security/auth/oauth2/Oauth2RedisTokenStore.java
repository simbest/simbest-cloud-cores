...
@Slf4j
@Component
public class Oauth2RedisTokenStore implements TokenStore {
    ...
    @Autowired
    public Oauth2RedisTokenStore(RedisConnectionFactory connectionFactory, AppConfig config) {
        this.connectionFactory = connectionFactory;
        //libeixiao
//        this.prefix = config.getRedisKeyPrefix();
        this.prefix = ApplicationConstants.EMPTY;
    }
...
//        }
    ...
    public OAuth2Authentication readAuthentication(String accessToken) {
        ...
        RedisConnection conn = getConnection();
        try {
            bytes = conn.get(serializeKey(AUTH + accessToken));
        } finally {
            conn.close();
        }
    ...
    public OAuth2AccessToken readAccessToken(String accessTokenValue) {
        byte[] key = serializeKey(ACCESS + accessTokenValue);
        byte[] bytes = null;
        RedisConnection conn = getConnection();
        try {
            bytes = conn.get(key);
    ...
    public OAuth2RefreshToken readRefreshToken(String refreshTokenValue) {
        byte[] key = serializeKey(REFRESH + refreshTokenValue);
        byte[] bytes = null;
        RedisConnection conn = getConnection();
        try {
            bytes = conn.get(key);
