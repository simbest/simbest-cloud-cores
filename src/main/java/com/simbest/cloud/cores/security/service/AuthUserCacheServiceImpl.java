...
public class AuthUserCacheServiceImpl implements IAuthUserCacheService {
    ...
    public static final Integer AUTH_USER_EXPIRE = 8;
    ...
    public void saveOrUpdateCacheUser(IUser user) {
        Assert.notNull(user, "用户实体不允许为空!");
        Assert.notNull(user.getId(), "用户主键唯一标识不允许为空!");
        Assert.notEmpty(user.getAuthRoles(), "缓存中用户角色不可为空");
        Assert.notEmpty(user.getAuthorities(), "缓存中用户角色及权限不可为空");
        if (null == user.getAuthPermissions() || user.getAuthPermissions().isEmpty()) {
            log.warn("即将在缓存中将用户权限置为空----SET AuthPermissions EMPTY----【{}】", user.getId());
        }
        //新增存放用户对象
        RedisUtil.setBeanGlobal(getUserCacheKey(user.getId()), user);
        RedisUtil.expire(getUserCacheKey(user.getId()), AUTH_USER_EXPIRE, TimeUnit.HOURS);
        log.trace("用户【{}】已通过键值【{}】完成缓存，并将在【{}】小时后失效", user, getUserCacheKey(user.getId()), AUTH_USER_EXPIRE);
    ...
    public IUser loadCacheUser(String keyword) {
        ...
        log.trace("通过关键字【{}】获取到用户主键ID为【{}】", getUserCacheKey(keyword), userId);
        if(StringUtils.isNotEmpty(userId)){
            IUser user = RedisUtil.getBeanGlobal(getUserCacheKey(userId), SimpleUser.class);
            if(null != user) {
                //获取用户基础信息时，数据只保留8小时，超时后失效时间
//                RedisUtil.expireGlobal(getUserCacheKey(user.getId()), AUTH_USER_EXPIRE, TimeUnit.HOURS);
//                log.trace("通过用户主键ID【{}】即将返回用户【{}】，并将于【{}】小时后失效", getUserCacheKey(userId), user, RedisUtil.getExpire(user.getId(), TimeUnit.HOURS));
                return user;
            }else{
                log.trace("无法通过用户主键ID【{}】读取用户信息", getUserCacheKey(userId));
                return null;
            }
        }else{
    ...
    public Set<IPermission> loadCacheUserPermission(String username, String appcode) {
        ...
        if(null != permissions) {
            log.trace("通过用户账号【{}】和应用标识【{}】即将返回用户权限个数为【{}】", username, appcode, permissions.size());
            return permissions;
        }
        else{
            log.trace("通过用户账号【{}】和应用标识【{}】无法读取用户权限", username, appcode);
            return null;
        }
    }
    ...
    public void removeCacheUserPermission(String username, String appcode) {
        ...
        log.trace("已清理键值【{}】的缓存", getUserPermissionCacheKey(username, appcode));
    }
    ...
    public Boolean loadCacheUserAccess(String username, String appcode) {
        ...
        if(null != isPermit) {
            log.trace("通过用户账号【{}】和应用标识【{}】即将返回用户访问应用结果为【{}】", username, appcode, isPermit);
            return isPermit;
        }
        else{
            log.trace("通过用户账号【{}】和应用标识【{}】无法读取用户访问应用结果", username, appcode);
            return null;
        }
    }
    ...
    public void removeCacheUserAccess(String username, String appcode) {
        ...
        log.trace("已清理键值【{}】的缓存", getAuthUserAccessGlobalKey(username, appcode));
    }
    ...
    public Boolean loadCacheUserPassword(String username, String password) {
        ...
        if(null != isRight) {
            log.trace("通过用户账号【{}】和密码【{}】即将返回用户访问应用结果为【{}】", username, password, isRight);
            return isRight;
        }
        else{
            log.trace("通过用户账号【{}】和密码【{}】在缓存校验失败", username, password);
            return null;
        }
    }
    ...
    public Set<String> getCacheUserPassword(String username) {
        ...
        //清理存放用户对象
        Set<String> userPasswordKeys = RedisUtil.globalKeys(AUTH_USER_PASSWORD_GLOBAL_KEY.concat(username).concat(ApplicationConstants.COLON).concat(ApplicationConstants.STAR));
        userPasswordKeys.forEach( k -> log.trace("用户的主数据、门户等密码为：【{}】", k));
