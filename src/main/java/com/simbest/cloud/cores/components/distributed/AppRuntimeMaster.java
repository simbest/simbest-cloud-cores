 ...
 */
package com.simbest.cloud.cores.components.distributed;

import com.simbest.cloud.cores.config.AppConfig;
import com.simbest.cloud.cores.constants.ApplicationConstants;
import com.simbest.cloud.cores.distributed.lock.DistributedRedisLock;
import com.simbest.cloud.cores.distributed.lock.DistributedRedissonLock;
import com.simbest.cloud.cores.utils.redis.RedisUtil;
import com.simbest.cloud.cores.utils.servers.HostUtil;
import com.simbest.cloud.cores.utils.servers.SocketUtil;
...
public class AppRuntimeMaster {
    ...
    public void becameMasertIfNotExist() {
        ...
        if(lock.isLocked()) {
            ...
            masterPort = RedisUtil.getBean(ApplicationConstants.MASTER_PORT, Integer.class);
            //1.没有Master
            if (StringUtils.isEmpty(masterHost) || null == masterPort || ZERO == masterPort) {
